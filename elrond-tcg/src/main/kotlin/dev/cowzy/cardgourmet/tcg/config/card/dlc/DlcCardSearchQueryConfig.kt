...
import dev.cowzy.cardgourmet.chef.commons.model.set.dlc.DlcFranchise
...
fun SearchQueryFilterBuilder.configureBasicDlcCardFilters() {
    filter("name", "n") {
    ...
    filter("franchise") {
        uuid(DlcCard::franchiseId, dlcPropertyKeys.FRANCHISE_ID)
        exactString(DlcFranchise::slug, dlcPropertyKeys.FRANCHISE_SLUG) {
            strict(true)
            autoValues(DlcFranchise::slug)
        }
        string(DlcFranchise::name, dlcPropertyKeys.FRANCHISE_NAME) { autoValues(DlcFranchise::name) }
    }
...
val dlcBasicCardSearchQueryConfig = SearchQuerySqlConfig(
    baseTable = DlcPrint::class,
    tableDependencies = tableDependencies,
    customFields = mapOf(
        "printId" to CustomField(DlcPrint::id),
        "language" to CustomField(DlcPrintTranslation::language, DlcCardTranslation::language),
    )
)
