...
private val properties = StaticValueProvider(allProperties.map { it.key }.toTypedArray())
private val mechanics = StaticValueProvider(allMechanics.map { it.key }.toTypedArray())
private val finishes = StaticValueProvider((MtgFinish.values().toList() - promoFinishesMapping.values.toSet()).map { it.getSerialName() }.toSet())
...
private val manaCardinalityMappings = mapOf(
...
private val mtgFinishMappings = mapOf(
    "foil" to "traditional_foil"
)

private val mtgLayoutMappings = mapOf(
    "art" to "art_series",
    "modal" to "modal_dfc",
...
private val mtgMediumMappings = mapOf(
    "online" to "mtgo"
)

private val mtgPropertyMappings = mapOf(
    "alchemy" to "arena",
...
private val mtgFrameEffectMappings = mapOf(
    "extended_art" to "extendedart"
)

private val mtgLanguageMappings = MtgLanguage.values().map { language ->
...
private val mtgMechanicsMappings = allMechanics
    .filter { it.key.contains("_") }
    .associate { it.key.replace("_", "") to it.key }
...
fun createMtgSearchQueryFilters(providers: MtgValueProviders): List<QueryFilter> {
...
val mtgSearchQueryConfig = SearchQueryConfig(
    table = MtgPrint::class,
    printIdColumn = MtgPrint::id,
    faceIndexColumn = MtgCardFace::index,
    languageColumns = arrayOf(UserCard::language, MtgPrintFaceTranslation::language, MtgCardFaceTranslation::language),
