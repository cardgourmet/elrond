...
private val finishes = StaticValueProvider(setOf("nonfoil", "foil"))
private val mediums = StaticValueProvider(setOf("paper"))
private val languages = StaticValueProvider(DlcLanguage.values().map { it.getSerialName() }.toSet())
private val inkTypes = StaticValueProvider(DlcInkType.values().map { it.getSerialName() }.toSet())

private val dlcLanguageMappings = DlcLanguage.values().associate { language ->
...
fun createDlcSearchQueryFilters(providers: DlcValueProviders): List<QueryFilter> {
...
val dlcSearchQueryConfig = SearchQueryConfig(
    table = DlcPrint::class,
    printIdColumn = DlcPrint::id,
    faceIndexColumn = null,
    languageColumns = arrayOf(UserCard::language, DlcPrintTranslation::language, DlcCardTranslation::language),
