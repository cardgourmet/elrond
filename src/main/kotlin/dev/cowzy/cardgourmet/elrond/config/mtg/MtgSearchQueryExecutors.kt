...
private fun createMtgBaseBuilder(config: SearchQueryConfig, fallbackFilter: QueryFilter): SearchQueryExecutorBuilder<MtgSearchQueryFlag> {
...
fun createMtgSearchQueryExecutor(providers: MtgValueProviders): SearchQueryExecutor<MtgSearchQueryFlag> {
    return createMtgBaseBuilder(mtgSearchQueryConfig, mtgNameFilter)
        .filters(createMtgSearchQueryFilters(providers))
        .build()
}

fun createMtgCollectionSearchQueryExecutor(providers: MtgValueProviders): SearchQueryExecutor<MtgSearchQueryFlag> {
    return createMtgBaseBuilder(mtgSearchQueryConfig, mtgCollectionNameFilter)
        .filters(createMtgCollectionSearchQueryFilters())
        .filters(createMtgSearchQueryFilters(providers))
        .build()
}

private fun <T> Iterable<T>.containsAny(vararg items: T) = items.any { this.contains(it) }
